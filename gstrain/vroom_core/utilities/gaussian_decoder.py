...
class GaussianDecoder(nn.Module):
    def __init__(self, feature_dim, anchor_cloud):
        ...
        self.covariance_network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                hidden_dim, self.number_gaussians_per_anchor * 7
            ),  # 3 for scale and 4 for rotation
        )
    ...
    def forward_pass(self, anchor_cloud, visible_anchors_mask, camera):
        ...
        anchor_to_camera_distance = visible_anchors - camera.camera_center.to(
            visible_anchors.device
        )  # distance matrix between each anchor and the camera
        viewing_vector = torch.nn.functional.normalize(
            anchor_to_camera_distance, dim=-1
        )  # direction vector from camera to anchor
        ...
        # predictions
        color_pred = self.color_network(latent).view(
            num_visible, self.number_gaussians_per_anchor, 3
        )
        opacity_pred = self.opacity_network(latent).view(
            num_visible, self.number_gaussians_per_anchor, 1
        )
        covariance_pred = self.covariance_network(latent).view(
            num_visible, self.number_gaussians_per_anchor, 7
        )

        # valid opacity mask
        negative_opacity_filter = (
            opacity_pred.squeeze(-1) > 0
        )  # we cant have negative opacities
        ...
        anchor_scale_repeated = anchor_scale_xyz.unsqueeze(1).expand(
            -1, self.number_gaussians_per_anchor, -1
        )
