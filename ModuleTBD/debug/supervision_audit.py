"""Supervision Audit — Verify supervision-view quality from the saved manifest.

Reads ``04_supervision_manifest.json`` (written when ``debug=True``) and runs:

1. **File-existence check** — every ``image_path`` entry must resolve on disk.
...
5. **Projection check** — if COLMAP seed points are loadable, projects them onto
   ...
   Saves a colour-coded overlay JPG per view (red=far, blue=near).

Output under ``<debug_dir>/`` (default ``<obj_dir>/05_supervision_audit/``)::
...
Usage (standalone)::
    ...
    python -m ModuleTBD.debug.supervision_audit \\
...
_SEED_DEPTH_MIN = 0.1   # must match dataset_builder._SEED_DEPTH_MIN
...
def check_file_existence(views):
    ...
    print(f"\n[1] File existence: {len(results)} views, {n_missing} missing")
    if n_missing:
        for r in results:
            if not r["exists"]:
                print(f"    MISSING view {r['view_idx']} ({r['source']}): {r['path']}")
    return results
...
def check_weight_distribution(views, debug_dir):
    ...
    print(f"    real       : {len(real_weights):>3} views,  total weight = {real_total:.3f}")
    print(f"    hallucinated: {len(halluc_weights):>3} views,  total weight = {halluc_total:.3f}")
    print(f"    ratio real/(real+halluc) = {real_total / max(grand_total, 1e-9):.3f}")
...
def check_coverage(views, debug_dir):
    ...
    print(f"\n[3] Coverage:  az_spread={az_spread:.1f}°  el_spread={el_spread:.1f}°")
...
# ── check 4: K/R/T sanity ─────────────────────────────────────────────────────

def check_krt_sanity(views):
    """Check rotation orthogonality, finite T, plausible K."""
    results = []
    print(f"\n[4] K/R/T sanity ({len(views)} views):")
    print(f"    {'#':>3}  {'Source':<12} {'R_err':>8}  {'T_norm':>8}  {'fx':>7}  {'fy':>7}  OK?")
    print("    " + "-" * 60)
    all_ok = True
    for i, v in enumerate(views):
...
def check_projections(views, xyz_W, debug_dir):
    ...
    print(f"\n[5] Projection check ({len(views)} views, {xyz.shape[0]} seed pts):")
    print(f"    {'#':>3}  {'Source':<12}  {'In-frame':>9}  {'Behind':>7}  {'CtrErr':>7}  Result")
    print("    " + "-" * 60)
    ...
    for i, v in enumerate(views):
        ...
        print(f"    {i:>3}  {src:<12}  {n_in_frame:>9}  {n_behind:>7}  {ctr:>7}  {flag}")
        results.append({
    ...
    print(f"    → {n_ok}/{len(results)} views have >10 seed points in frame")
    if n_ok < len(results) // 2:
        print("    *** WARNING: majority of views have few in-frame points — "
...
def generate_debug_artifacts(*, obj_dir, scope=None, frame=None,
                              model_path=None, object_id=None, debug_dir):
    """Audit the supervision manifest and write outputs to *debug_dir*.

    Parameters
    ----------
    obj_dir      : object pipeline output dir (contains 04_supervision_manifest.json)
    scope        : ObjectScope (used for COLMAP loading; optional)
    frame        : ObjectFrame (unused currently; reserved)
    model_path   : path to the trained gstrain model directory (enables projection check)
    object_id    : integer object label id (enables projection check)
    debug_dir    : output directory for audit artifacts
    """
    ...
    print(f"\n{'=' * 65}")
    print("SUPERVISION AUDIT")
    print(f"{'=' * 65}")
    print(f"  Manifest: {obj_dir / '04_supervision_manifest.json'}")
    print(f"  Views: {len(views)}  (real={manifest.get('n_real', '?')}, "
          f"hallucinated={manifest.get('n_hallucinated', '?')})")

    report: dict = {"n_views": len(views)}
    ...
    # 5 – projection check (optional: needs COLMAP points)
    if model_path is not None and object_id is not None and scope is not None:
        ...
        print("\n[5] Projection check: skipped (model_path/object_id/scope not provided)")
...
def _parse_args():
    p = argparse.ArgumentParser(description="ModuleTBD supervision audit.")
    p.add_argument("--obj_dir", required=True,
                   help="Object pipeline output dir (contains 04_supervision_manifest.json)")
    p.add_argument("--model_path", default=None,
                   help="Trained gstrain model path (enables projection check)")
    p.add_argument("--object_id", type=int, default=None,
                   help="Integer object label id (enables projection check)")
    p.add_argument("--debug_dir", default=None,
                   help="Output dir (default: <obj_dir>/05_supervision_audit)")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    args = _parse_args()
    debug_dir = args.debug_dir or str(Path(args.obj_dir) / "05_supervision_audit")

    scope = None
    if args.model_path and args.object_id is not None:
        try:
            from ModuleTBD.utils.scene_analysis import compute_object_scope
            scope, _frame, _pipe_config = compute_object_scope(args.model_path, args.object_id)
