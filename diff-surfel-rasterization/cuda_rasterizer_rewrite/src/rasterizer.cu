...
)
{
    ...
    CUDA_SAFE_CALL(FWD::render(
                       ...
                       colors_feat.contiguous().data_ptr<float>(),
                       background.contiguous().data_ptr<float>(),
                       reinterpret_cast<float2 *>(projected_centers.contiguous().data_ptr<float>()),
                       reinterpret_cast<float3 *>(splat2pix_mats.contiguous().data_ptr<float>()),
                       reinterpret_cast<float4 *>(normal_opacity.contiguous().data_ptr<float>()),
                       reinterpret_cast<uint32_t *>(sorted_surfel_indices.contiguous().data_ptr<int32_t>()),
                       reinterpret_cast<uint2 *>(tile_ranges.contiguous().data_ptr<int32_t>()),
...
)
{
    ...
    PytorchAllocators::BackwardAllocationContext ctx(
        points_world_space.options(), num_color_feat_channels, surfel_count);

    // Call backward orchestrator
    if (surfel_count > 0)
    {
        RasterizerOrchestrator::backward(
            // Forward pass saved state
            surfel_count,
            reinterpret_cast<float3 *>(points_world_space.contiguous().data_ptr<float>()),
            reinterpret_cast<float2 *>(scale_vecs.contiguous().data_ptr<float>()),
            ...
            reinterpret_cast<float3 *>(ctx.grad_splat2pix_mats.data_ptr<float>()),
...
)
{
    ...
    if (surfel_count > 0)
    {
        ...
        CUDA_SAFE_CALL(BWD::render(
                           ...
                           grad_rendered_aux.contiguous().data_ptr<float>(),
                           // Output gradients (to pytorch).
                           // For now all of them are except grad_colors_feat
