...
def fuse_tsdf(depth_maps, intrinsics_list, extrinsics_list, grid_shape, 
    ...
    # 2. Pre-compute homogeneous world coordinates, same for all cameras,(N³, 4)
    coords_x = torch.arange(grid_shape[0], device=device, dtype=torch.float32) * voxel_size + grid_origin[0]
    coords_y = torch.arange(grid_shape[1], device=device, dtype=torch.float32) * voxel_size + grid_origin[1]
    coords_z = torch.arange(grid_shape[2], device=device, dtype=torch.float32) * voxel_size + grid_origin[2]
    
    grid_x, grid_y, grid_z = torch.meshgrid(coords_x, coords_y, coords_z, indexing='ij')
    
    world_points = torch.stack([grid_x, grid_y, grid_z], dim=-1).view(-1, 3)
    ...
    for i in range(num_cameras):
        ...
        tsdf_i, color_i, weight_i = generate_tsdf_single_cam(
            ...
            grid_shape, voxel_size, trunc_margin, precomputed_wph_t, color_image=colors_t
        )

        tsdf_sum += tsdf_i * weight_i # grid of tsdf values
    ...
    fused_tsdf = torch.ones_like(tsdf_sum) # default to 1.0 (empty space)
    fused_tsdf[valid_mask] = tsdf_sum[valid_mask] / weight_sum[valid_mask]
    fused_colors = None
    if color_images is not None:
        fused_colors = torch.zeros_like(color_sum)
        fused_colors[valid_mask] = color_sum[valid_mask] / weight_sum[valid_mask].unsqueeze(-1) # weighted average of colors
