...
def main():
    ...
    parser.add_argument(
        "--data_path",
        required=True,
        help="Scene folder containing images/ (read-only input)",
    )
    parser.add_argument(
        "--output_path", help="Path to save all outputs (defaults to data_path)"
    )
    ...
    parser.add_argument(
        "--force_colmap", action="store_true", help="Pass --force to colmap_runner.py"
    )
    parser.add_argument("--skip_colmap", action="store_true", help="Skip COLMAP step")
    parser.add_argument(
        "--skip_masks", action="store_true", help="Skip mask_processor step"
    )
    parser.add_argument(
        "--skip_tracking", action="store_true", help="Skip object_tracker step"
    )
    ...
    parser.add_argument(
        "--sam_ckpt",
        default="Module-1/models/sam3.pt",
        help="SAM3 checkpoint (e.g., sam3.pt, sam3_b.pt)",
    )
    ...
    parser.add_argument(
        "--disable_motion_comp",
        action="store_true",
        help="Disable global camera-motion compensation in tracker",
    )
    parser.add_argument(
        "--disable_consensus",
        action="store_true",
        help="Disable in-clip temporal consensus in tracker",
    )
    parser.add_argument(
        "--consensus_window",
        type=int,
        default=8,
        help="Temporal window length for tracker consensus",
    )
    parser.add_argument(
        "--consensus_tie_margin",
        type=float,
        default=0.05,
        help="IoU vote margin for appearance tie-break",
    )

    # Voting
    parser.add_argument(
        "--algorithm", default="majority", choices=["majority", "prob", "corr"]
    )
    parser.add_argument(
        "--output_dir",
        default="labeled_output",
        help="Output dir name inside output_path for vote.py",
    )
    parser.add_argument("--min_points", type=int, default=10)
    parser.add_argument(
        "--disable_alias_merge",
        action="store_true",
        help="Disable correspondence-based alias merging in vote.py",
    )
    parser.add_argument(
        "--alias_iou_thresh",
        type=float,
        default=0.40,
        help="Min 3D IoU to consider two tracker IDs as aliases",
    )
    parser.add_argument(
        "--alias_min_covisibility",
        type=int,
        default=15,
        help="Min shared views for alias merge candidates",
    )
    ...
    if reconstruction_mode is None:
        ...
        if (data_path / "manifest.json").exists():
            print(
                "ARCore manifest detected — using standard SfM with post-hoc metric alignment."
            )
    ...
    if not args.skip_masks:
        masks_cmd = [
            ...
            str(paths["images"]),
            "--output_dir",
            str(paths["sam_output"]),
            "--sam_ckpt",
            args.sam_ckpt,
            "--device",
            args.device,
            "--ultralytics_home",
            args.ultralytics_home,
            "--text_prompts",
            *args.text_prompts,
            "--min_mask_area",
            ...
            str(args.border_touch_threshold),
            "--merge_thresh",
            str(args.merge_thresh),
            "--proximity_gap",
            str(args.proximity_gap),
            "--proximity_color_thresh",
            str(args.proximity_color_thresh),
        ]
    ...
    if not args.skip_tracking:
        tracking_cmd = [
            ...
            str(args.ema),
            "--reid_threshold",
            str(args.reid_threshold),
            "--consensus_window",
            str(args.consensus_window),
            "--consensus_tie_margin",
            str(args.consensus_tie_margin),
        ]
    ...
    if not args.skip_voting:
        voting_cmd = [
            ...
            str(args.min_points),
            "--alias_iou_thresh",
            str(args.alias_iou_thresh),
            "--alias_min_covisibility",
            str(args.alias_min_covisibility),
        ]
