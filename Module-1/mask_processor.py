...
def filter_background_masks(
    ...
    border_ratio = (top_hits + bottom_hits + left_hits + right_hits) / max(
        1.0, border_len
    )
...
def merge_overlapping_masks(
    masks: List[np.ndarray], thresh: float = 0.78
) -> List[np.ndarray]:
...
def split_disconnected(
    masks: List[np.ndarray], min_component_area: int = 1
) -> List[np.ndarray]:
...
def draw_mask_overlay(
    image: np.ndarray, masks: List[np.ndarray], alpha: float = 0.5
) -> np.ndarray:
...
def run_pipeline(args):
    ...
    logger.info(
        f"Processing {len(image_paths)} frames with SAM3 text prompting + postprocessing..."
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="VRoom SAM3 Text-Prompt Mask Processor"
    )
    parser.add_argument(
        "--input_dir", required=True, help="Path to input images directory"
    )
    parser.add_argument(
        "--output_dir", required=True, help="Path to save masks and visualizations"
    )
    parser.add_argument(
        "--sam_ckpt",
        default="Module-1/models/sam3.pt",
        help="Ultralytics SAM3 checkpoint name or .pt path",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Minimum kept mask area in pixels",
    )
    parser.add_argument(
        "--max_area_ratio",
        type=float,
        default=0.50,
        help="Drop masks larger than this image area ratio",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.78,
        help="Containment threshold for overlap merge",
    )
    parser.add_argument(
        "--proximity_gap",
        type=int,
        default=20,
        help="Pixel gap threshold for proximity merge",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable splitting disconnected components",
    )
